...
function PublicKey(Q) {
    ...
    // /**
    //     @todo secp224r1
    //     @return {string} PUB_R1_base58pubkey..
    // */
    // function toString() {
    //     if(pubdata) {
    //         return pubdata
    //     }
    //     pubdata = `PUB_R1_` + keyUtils.checkEncode(toBuffer(), 'R1')
    //     return pubdata;
    // }
    ...
    */
    ...
    function toUncompressed() {
        ...
        var point = ecurve.Point.decodeFrom(secp256r1, buf);
    ...
    function child(offset) {
        ...
        if (secp256r1.isInfinity(Qprime)) throw new Error("Child offset derived to an invalid key, try again");
...
PublicKey.fromBuffer = function (buffer) {
    return PublicKey(ecurve.Point.decodeFrom(secp256r1, buffer));
};
...
PublicKey.fromStringOrThrow = function (public_key) {
    ...
    assert(match.length === 3, 'Expecting public key like: PUB_R1_base58pubkey..');
