...
function PrivateKey(d) {
    ...
    /** @return {string} private key like PVT_R1_base58privatekey.. */
    function toString() {
        // todo, use PVT_R1_
        // return 'PVT_R1_' + keyUtils.checkEncode(toBuffer(), 'R1')
        return toWif();
    }
    ...
    */
    function getSharedSecret(public_key) {
        ...
        var KBP = Point.fromAffine(secp256r1, BigInteger.fromBuffer(KB.slice(1, 33)), // x
...
function parseKey(privateStr) {
    ...
    assert(match.length === 3, 'Expecting private key like: PVT_R1_base58privateKey..');

    var _match = _slicedToArray(match, 3),
        keyType = _match[1],
        keyString = _match[2];

    assert.equal(keyType, 'R1', 'R1 private key expected');
...
function unitTest() {
    ...
    // assert.equal(pvt.toString(), 'PVT_R1_2jH3nnhxhR3zPUcsKaWWZC9ZmZAnKm3GAnFD1xynGJE1Znuvjd', pvtError)
    ...
    // assert.equal(pub.toString(), 'PUB_R1_859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2Ht7beeX', pubError)
