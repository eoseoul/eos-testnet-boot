...
var curve = require('ecurve').getCurveByName('secp256r1');
...
function Signature(r, s, i) {
    ...
    */
    ...
    function toString() {
        ...
        signatureCache = 'SIG_R1_' + keyUtils.checkEncode(toBuffer(), 'R1');
        return signatureCache;
    }
...
/**
    @arg {string} signature - like SIG_R1_base58signature..
...
Signature.fromStringOrThrow = function (signature) {
    ...
    assert(match != null && match.length === 3, 'Expecting signature like: SIG_R1_base58signature..');

    var _match = _slicedToArray(match, 3),
        keyType = _match[1],
        keyString = _match[2];

    assert.equal(keyType, 'R1', 'R1 signature expected');
